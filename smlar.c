...
getDefaultOpclass(Oid amoid, Oid typid)
{
	...
	heapRel = table_open(OperatorClassRelationId, AccessShareLock);
...
getAMProc(Oid amoid, Oid typid)
{
	...
	heapRel = table_open(AccessMethodProcedureRelationId, AccessShareLock);
	ScanKeyInit(&skey[0],
